...
namespace Imperium.Framework.Data
{
    ...
    public class NLogCommandInterceptor : IDbCommandInterceptor, IDbConnectionInterceptor, IDbTransactionInterceptor
    {
        ...
        public NLogCommandInterceptor(DbContext context, ILogger logger) : this(logger)
        {
            if (context == null)
            {
        ...
        /// being logged.
        /// </summary>
        protected virtual DbContext Context => _context != null && _context.IsAlive ? (DbContext)_context.Target : null;
        ...
        /// </summary>
        protected virtual Stopwatch Stopwatch { get; } = new Stopwatch();
        ...
        public virtual void LogCommand<TResult>(DbCommand command, DbCommandInterceptionContext<TResult> interceptionContext)
        {
            ...
            LogInformation(interceptionContext.IsAsync ?
        ...
        public virtual void LogResult<TResult>(DbCommand command, DbCommandInterceptionContext<TResult> interceptionContext)
        {
            ...
            {
                LogError(interceptionContext.Exception, $"Failed in {Stopwatch.ElapsedMilliseconds} ms with error: {interceptionContext.Exception.Message}");
            }
            else if (interceptionContext.TaskStatus.HasFlag(TaskStatus.Canceled))
            {
                LogInformation($"Canceled in {Stopwatch.ElapsedMilliseconds} ms");
            }
            else
            {
                ...
                var resultString = (object)result == null
                    ...
                    : (result is DbDataReader)
                        ? result.GetType().Name
                        : result.ToString();
                LogInformation($"Completed in {Stopwatch.ElapsedMilliseconds} ms with result: {resultString}");
            }
        }
        ...
        public virtual void Opened(DbConnection connection, DbConnectionInterceptionContext interceptionContext)
        {
            if (ShouldLog(interceptionContext))
            {
                if (interceptionContext.Exception != null)
                {
                    LogError(interceptionContext.Exception,
                        interceptionContext.IsAsync ?
                            $"Failed to open connection asynchronously with error: {interceptionContext.Exception.Message}" :
                            $"Failed to open connection with error: {interceptionContext.Exception.Message}");
                }
                else if (interceptionContext.TaskStatus.HasFlag(TaskStatus.Canceled))
                {
                    LogInformation("Canceled open connection");
                }
                else
                {
                    LogInformation(
                        interceptionContext.IsAsync ?
                            "Opened connection asynchronously" :
                            "Opened connection");
                }
            }
        }
        ...
        public virtual void Closed(DbConnection connection, DbConnectionInterceptionContext interceptionContext)
        {
            if (ShouldLog(interceptionContext))
            {
                if (interceptionContext.Exception != null)
                {
                    LogError(
                        interceptionContext.Exception,
                        $"Failed to close connection with error: {interceptionContext.Exception.Message}");
                }
                else
                {
                    LogInformation("Closed connection");
                }
            }
        }
        ...
        public virtual void Disposing(DbConnection connection, DbConnectionInterceptionContext interceptionContext)
        {
            ...
            {
                LogInformation("Disposed connection");
            }
        }
        ...
        public virtual void RolledBack(DbTransaction transaction, DbTransactionInterceptionContext interceptionContext)
        {
            if (ShouldLog(interceptionContext))
            {
                if (interceptionContext.Exception != null)
                {
                    LogError(
                        interceptionContext.Exception,
                        $"Failed to rollback transaction at {DateTimeOffset.Now} with error: {interceptionContext.Exception.Message}");
                }
                else
                {
                    LogInformation($"Rolled back transaction at {DateTimeOffset.Now}");
                }
            }
        }
    }
